...
def _preprocess_qkv(
    ...
    if (
        k.shape[0] != batch
        or k.shape[2] != seq_len
        or k.shape[3] != head_dim
    ):
        raise ValueError(
            "q, k, and v must have the same batch, sequence, and head dimensions, "
    ...
    qm_grouped = qm.reshape(
        batch, num_kv_heads, q_heads_per_kv_head, qm.shape[2], head_dim
    )
    qk_correction = torch.matmul(
        qm_grouped.to(torch.float32),
        k.transpose(-2, -1).unsqueeze(2).to(torch.float32),
    ).reshape(batch, num_q_heads, qm.shape[2], seq_len)
...
def nvfp4_attention_sm120_quantize_qkv(
    ...
    contiguous CUDA tensors with the same dtype and device. K and V must have
    the same shape; Q may have more heads when its head count is divisible by
    the K/V head count. The sequence dimension is padded to a multiple of 128
    ...
    Tuple[torch.Tensor, ...]
        ...
        ``[batch, num_q_heads, seq_len / 128, seq_len]``
        (``[batch, num_q_heads, 1, seq_len]`` when
...
def _check_inputs(
    ...
    if (
        k_fp4.ndim != 4
        or k_fp4.shape[0] != batch
        or k_fp4.shape[2] != seq_len
        or k_fp4.shape[3] != packed_head_dim
    ):
        raise ValueError(
            "k_fp4 must have shape "
            f"[{batch}, num_kv_heads, {seq_len}, {packed_head_dim}], "
            f"got {tuple(k_fp4.shape)}"
        )
    ...
    expected_q_scale = (batch, num_q_heads, seq_len, head_dim // 16)
...
def nvfp4_attention_sm120_fwd(
    ...
    :func:`nvfp4_attention_sm120_quantize_qkv`. ``q_fp4`` uses Q heads while
    ``k_fp4``, ``v_fp4_t``, and their scales use K/V heads. ``v_fp4_t`` and
    ``v_scale_t`` are stored with their final two dimensions transposed.
    ...
    batch, num_q_heads, _, seq_len, head_dim = _check_inputs(
        q_fp4,
        k_fp4,
        v_fp4_t,
        q_scale,
    ...
    if out is None:
        ...
        if tuple(out.shape) != (batch, num_q_heads, seq_len, head_dim):
            raise ValueError(
                ...
                f"{(batch, num_q_heads, seq_len, head_dim)}"
            )
