...
from .jit.nvfp4_attention import gen_nvfp4_attention_sm120_module
...
# Kernel tensor_layout=1 corresponds to [batch, heads, seqlen, head_dim].
_HND_LAYOUT = 1
...
def _pad_seqlen_to_128(x: torch.Tensor) -> torch.Tensor:
...
def _preprocess_qkv(
    ...
    for name, tensor in (("q", q), ("k", k), ("v", v)):
        ...
        if tensor.ndim != 4:
            raise ValueError(
                f"{name} must have shape [batch, heads, seqlen, head_dim], "
    ...
    if per_block_mean:
        num_groups = seqlen // _TOKEN_BLOCK_SIZE
        q_grouped = q.reshape(batch, heads, num_groups, _TOKEN_BLOCK_SIZE, head_dim)
        qm = q_grouped.mean(dim=3)
        q = (
            (q_grouped - qm.unsqueeze(3))
            .reshape(batch, heads, seqlen, head_dim)
            .contiguous()
        )
    else:
...
@flashinfer_api
def nvfp4_attention_quantize_qkv(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    ...
    The input layout is ``[batch, heads, seqlen, head_dim]``. Inputs must be
    ...
    Tuple[torch.Tensor, ...]
        ...
        ``q_scale``, ``k_scale``, ``v_scale_t``, and the FP32 QK correction.
    """
    q_proc, k_proc, v_proc, qk_correction = _preprocess_qkv(q, k, v, per_block_mean)
    batch, heads, seqlen, head_dim = q_proc.shape

    q_fp4 = torch.empty(
        (batch, heads, seqlen, head_dim // 2), device=q.device, dtype=torch.uint8
    )
    k_fp4 = torch.empty_like(q_fp4)
    v_fp4_t = torch.empty(
        (batch, heads, head_dim, seqlen // 2), device=q.device, dtype=torch.uint8
    )
    q_scale = torch.empty(
        (batch, heads, seqlen, head_dim // 16),
        device=q.device,
        dtype=torch.float8_e4m3fn,
    )
    k_scale = torch.empty_like(q_scale)
    v_scale_t = torch.empty(
        (batch, heads, head_dim, seqlen // 16),
        device=q.device,
        dtype=torch.float8_e4m3fn,
    )
...
def _check_inputs(
    ...
    if q_fp4.ndim != 4:
        raise ValueError("q_fp4 must have shape [batch, heads, seqlen, head_dim / 2]")
    if k_fp4.shape != q_fp4.shape:
        raise ValueError(
            f"k_fp4 shape {tuple(k_fp4.shape)} must match q_fp4 {tuple(q_fp4.shape)}"
        )

    batch, heads, seqlen, packed_head_dim = q_fp4.shape
    head_dim = packed_head_dim * 2
    if head_dim not in _SUPPORTED_HEAD_DIMS:
        raise ValueError(f"head_dim must be 64 or 128, got {head_dim}")
    if seqlen % _TOKEN_BLOCK_SIZE != 0:
        raise ValueError(f"seqlen must be padded to a multiple of 128, got {seqlen}")
    if head_dim % 16 != 0:
        raise ValueError(f"head_dim must be divisible by 16, got {head_dim}")

    expected_v = (batch, heads, head_dim, seqlen // 2)
    if tuple(v_fp4_t.shape) != expected_v:
        raise ValueError(f"v_fp4_t shape {tuple(v_fp4_t.shape)} must be {expected_v}")

    expected_sf_qk = (batch, heads, seqlen, head_dim // 16)
    if tuple(q_scale.shape) != expected_sf_qk:
        raise ValueError(f"q_scale shape {tuple(q_scale.shape)} must be {expected_sf_qk}")
    if tuple(k_scale.shape) != expected_sf_qk:
        raise ValueError(f"k_scale shape {tuple(k_scale.shape)} must be {expected_sf_qk}")

    expected_v_scale = (batch, heads, head_dim, seqlen // 16)
    ...
    if (
        ...
        or qk_correction.shape[1] != heads
    ):
        raise ValueError("qk_correction must have shape [batch, heads, q_groups, seqlen]")
    expected_delta_groups = seqlen // _TOKEN_BLOCK_SIZE if per_block_mean else 1
    if qk_correction.shape[2] != expected_delta_groups:
        raise ValueError(
            f"qk_correction q_groups dimension must be {expected_delta_groups}, "
            f"got {qk_correction.shape[2]}"
        )
    if qk_correction.shape[-1] != seqlen:
        raise ValueError(
            f"qk_correction last dimension must be {seqlen}, got {qk_correction.shape[-1]}"
        )

    return batch, heads, seqlen, head_dim


@flashinfer_api
def nvfp4_attention_fwd(
    q_fp4: torch.Tensor,
    k_fp4: torch.Tensor,
    v_fp4_t: torch.Tensor,
    q_scale: torch.Tensor,
    ...
    :func:`nvfp4_attention_quantize_qkv`. ``q_fp4`` and ``k_fp4`` use layout
    ``[batch, heads, seqlen, head_dim / 2]``; ``v_fp4_t`` and ``v_scale_t`` are
    stored transposed as ``[batch, heads, head_dim, packed_seqlen]``.
    ...
    qk_correction : torch.Tensor
        FP32 correction term returned by ``nvfp4_attention_quantize_qkv``.
    ...
    per_block_mean : bool, optional
        Must match the value used by ``nvfp4_attention_quantize_qkv``.
    ...
    batch, heads, seqlen, head_dim = _check_inputs(
        q_fp4,
        k_fp4,
        v_fp4_t,
        q_scale,
    ...
    if out is None:
        ...
        if tuple(out.shape) != (batch, heads, seqlen, head_dim):
            raise ValueError(
                f"out shape {tuple(out.shape)} must be {(batch, heads, seqlen, head_dim)}"
            )
    ...
    if lse is None:
        lse = torch.empty((batch, heads, seqlen), device=q_fp4.device, dtype=torch.float32)
    else:
        _check_cuda_contiguous("lse", lse)
        _check_same_device("lse", lse, "q_fp4", q_fp4)
        if tuple(lse.shape) != (batch, heads, seqlen):
            raise ValueError(f"lse shape {tuple(lse.shape)} must be {(batch, heads, seqlen)}")
