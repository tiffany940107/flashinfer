...
def gen_gemm_sm100_module_cutlass_fp4() -> JitSpec:
    ...
    # Keep the incumbent TMA epilogue as the fallback for outputs that do not
    # satisfy the no-smem epilogue's 256-bit base/stride alignment contract.
    kernel_templates = [
        ("fp4_gemm_cutlass.jinja", ""),
        ("fp4_gemm_cutlass_sm10x_generic_store256.jinja", "_store256"),
    ]
    for template_name, file_suffix in kernel_templates:
