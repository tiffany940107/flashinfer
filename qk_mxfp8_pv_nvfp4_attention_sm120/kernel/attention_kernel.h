...
inline constexpr int kProducerRegisters = 24;
  ...
  if (warp_group_role == WarpGroupRole::Producer) {
    ...
    bool const defer_q_release = [&] {
      if constexpr (std::is_same_v<TileScheduler,
                                   qk_mxfp8_pv_nvfp4_attention::SingleTileScheduler>) {
        return false;
      } else {
        return !Is_causal && scheduler_params.total_blocks > int(gridDim.x);
      }
    }();
    bool q_release_deferred = false;
    ...
    for (auto work_tile_info = scheduler.get_initial_work();
      ...
      if (defer_q_release && q_release_deferred) {
        ...
        ++smem_pipe_read_q;
        q_release_deferred = false;
      }

      Tensor tOrO = partition_fragment_C(tiled_mma_pv,
                                         make_shape(Int<kBlockMPerWG>{}, Int<Ktraits::kHeadDim>{}));

      qk_mxfp8_pv_nvfp4_attention::SoftmaxFused<2 * (2 * kBlockMPerWG / NumMmaThreads)>
          softmax_fused;
