...
DEFAULT_CONFIGS = (
    # R20 comparison points.
    # B=4,H=8,S=4096,D=128 non-causal
    (4, 8, 4096, 128, False),
    # B=1,H=8,S=32768,D=128 non-causal
    (1, 8, 32768, 128, False),
)
PER_BLOCK_MEAN = True
CSV_FIELDS = (
...
def skip_unless_sm120() -> None:
    ...
    if capability != (12, 0):
        ...
        print("Skipping: NVFP4 attention SM120 benchmark requires compute capability (12, 0).")
        sys.exit(0)
...
def attention_flops(config: BenchConfig) -> float:
    # Conventional dense self-attention throughput denominator:
    # - Count one multiply-add as 2 FLOPs.
    # - Non-causal attention computes QK^T and P*V over all S*S score entries:
    #   2 * B*H*S*S*D for QK^T plus 2 * B*H*S*S*D for P*V.
    # - Causal attention computes the triangular half, matching the existing
    #   FlashInfer benchmark convention: half of the non-causal denominator.
...
def bench_config(
    ...
    # Compile/load and prepare quantized inputs outside the attention-only timer.
    quantized_qkv = flashinfer.nvfp4_attention_quantize_qkv(
        q, k, v, per_block_mean=PER_BLOCK_MEAN
    )
    flashinfer.nvfp4_attention_fwd(
        *quantized_qkv,
        sm_scale=sm_scale,
    ...
    def attention_only():
        return flashinfer.nvfp4_attention_fwd(
            *quantized_qkv,
            sm_scale=sm_scale,
            causal=config.causal,
            per_block_mean=PER_BLOCK_MEAN,
    ...
    def end_to_end():
        qkv = flashinfer.nvfp4_attention_quantize_qkv(
            q, k, v, per_block_mean=PER_BLOCK_MEAN
        )
        return flashinfer.nvfp4_attention_fwd(
            *qkv,
    ...
    # FlashInfer JIT modules use TVM FFI. With normal CUDA event timing, the
    # CPU launch/FFI gap between the start event and kernel launch is visible,
    # which heavily distorts short-sequence kernel throughput. CUDA Graph replay
    # measures the steady-state GPU work and is the right comparison point for
    # the original R20 kernel latency. Capture one attention invocation per graph
    # replay to avoid shifting cache behavior across back-to-back graph-internal
    # repetitions.
