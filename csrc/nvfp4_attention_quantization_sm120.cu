...
#define DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, ...)              \
  if (head_dim == 64) {                                         \
    ...
    TVM_FFI_ICHECK(false) << "Unsupported head dim: "           \
                          << int(head_dim);                     \
  }
...
#define CHECK_QUANT_DIMS(x, true_dim) \
  TVM_FFI_ICHECK_EQ((x).ndim(), true_dim) << #x " rank mismatch"
...
__global__ void scaled_fp4_quant_kernel(
  ...
  if constexpr (CVT_FP4_ELTS_PER_THREAD == 16) {
    ...
    uint32_t offset_local = (col_id_local / 4) * 256 + (col_id_local % 4) +
                            (token_id_local / 16) * 64 + (token_id_local % 16) * 4;
...
__global__ void scaled_fp4_quant_trans_kernel(
  ...
  if constexpr (CVT_FP4_ELTS_PER_THREAD == 16) {
    ...
    uint32_t offset_local = (col_id_local / 4) * 256 + (col_id_local % 4) +
                            (row_id_local / 16) * 64 + (row_id_local % 16) * 4;
    reinterpret_cast<uint8_t*>(output_sf_save_base + offset_local)[0] = SFValueFP8;
  } else {
    if (threadIdx.x % 2 == 0) {
...
void scaled_fp4_quant_permute(TensorView input, TensorView output, TensorView output_sf,
  ...
  if (tensor_layout == 0) {
    ...
    CHECK_QUANT_SHAPE(output, batch_size,
    ...
    CHECK_QUANT_SHAPE(output, batch_size, num_heads,
                      ((num_tokens + BLOCK_SIZE - 1) / BLOCK_SIZE) * BLOCK_SIZE,
                      head_dim / 2);
