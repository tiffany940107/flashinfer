...
#include <flashinfer/nvfp4_attention/api/launcher.h>
...
void set_params_fprop(Flash_fwd_params& params, TensorView q, TensorView k, TensorView v,
                      ...
                      TensorView qk_correction, TensorView out, TensorView lse,
  ...
  const int seqlen_k = static_cast<int>(k.size(2));
  ...
  params.unpadded_seqlen_k = seqlen_k;
  ...
  params.head_divmod = cutlass::FastDivmod(heads);
...
void run_mha_fwd_dispatch_dtype(Flash_fwd_params& params, cudaStream_t stream) {
  ...
  if (params.d == 64) {
    sage::run_mha_fwd_<cutlass::nv_float4_t<cutlass::float_e2m1_t>, 64, OType>(params, stream);
  } else if (params.d == 128) {
    sage::run_mha_fwd_<cutlass::nv_float4_t<cutlass::float_e2m1_t>, 128, OType>(params, stream);
  } else {
...
void fwd(TensorView q_fp4, TensorView k_fp4, TensorView v_fp4_t, TensorView q_scale,
  ...
  TVM_FFI_ICHECK_EQ(v_scale_t.dtype(), dl_float8_e4m3fn)
      << "v_scale_t must be float8_e4m3fn";
  ...
  const int64_t heads = q_fp4.size(1);
  const int64_t seqlen = q_fp4.size(2);
  const int64_t head_dim = q_fp4.size(3) * 2;

  TVM_FFI_ICHECK(head_dim == 64 || head_dim == 128) << "head_dim must be 64 or 128";
  TVM_FFI_ICHECK_EQ(seqlen % 128, 0) << "seqlen must be a multiple of 128";

  TVM_FFI_ICHECK_EQ(k_fp4.size(0), batch);
  TVM_FFI_ICHECK_EQ(k_fp4.size(1), heads);
  TVM_FFI_ICHECK_EQ(k_fp4.size(2), seqlen);
  TVM_FFI_ICHECK_EQ(k_fp4.size(3), q_fp4.size(3));

  TVM_FFI_ICHECK_EQ(v_fp4_t.size(0), batch);
  TVM_FFI_ICHECK_EQ(v_fp4_t.size(1), heads);
  TVM_FFI_ICHECK_EQ(v_fp4_t.size(2), head_dim);
  TVM_FFI_ICHECK_EQ(v_fp4_t.size(3), seqlen / 2);

  TVM_FFI_ICHECK_EQ(q_scale.size(0), batch);
  TVM_FFI_ICHECK_EQ(q_scale.size(1), heads);
  TVM_FFI_ICHECK_EQ(q_scale.size(2), seqlen);
  TVM_FFI_ICHECK_EQ(q_scale.size(3), head_dim / 16);
  TVM_FFI_ICHECK_EQ(k_scale.size(0), batch);
  TVM_FFI_ICHECK_EQ(k_scale.size(1), heads);
  TVM_FFI_ICHECK_EQ(k_scale.size(2), seqlen);
  TVM_FFI_ICHECK_EQ(k_scale.size(3), head_dim / 16);
  TVM_FFI_ICHECK_EQ(v_scale_t.size(0), batch);
  TVM_FFI_ICHECK_EQ(v_scale_t.size(1), heads);
  TVM_FFI_ICHECK_EQ(v_scale_t.size(2), head_dim);
  TVM_FFI_ICHECK_EQ(v_scale_t.size(3), seqlen / 16);

  TVM_FFI_ICHECK_EQ(qk_correction.size(0), batch);
  TVM_FFI_ICHECK_EQ(qk_correction.size(1), heads);
  TVM_FFI_ICHECK_EQ(qk_correction.size(3), seqlen);

  TVM_FFI_ICHECK_EQ(out.size(0), batch);
  TVM_FFI_ICHECK_EQ(out.size(1), heads);
  TVM_FFI_ICHECK_EQ(out.size(2), seqlen);
  TVM_FFI_ICHECK_EQ(out.size(3), head_dim);
  TVM_FFI_ICHECK_EQ(lse.size(0), batch);
  TVM_FFI_ICHECK_EQ(lse.size(1), heads);
  TVM_FFI_ICHECK_EQ(lse.size(2), seqlen);
  ...
  set_params_fprop(params, q_fp4, k_fp4, v_fp4_t, q_scale, k_scale, v_scale_t,
                   qk_correction, out, lse, static_cast<float>(sm_scale), causal,
                   per_block_mean);
